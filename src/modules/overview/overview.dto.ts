...
export interface OverviewDTO {
  ...
  // retrofit-81: each point carries `approx` — true when its value (for the aggregate, ANY
  // contributing portfolio's forward-filled snapshot) comes from a backfilled ESTIMATE rather
  // than a real daily observation. The frontend renders the approx segment distinctly (dashed/
  // "estimated history") so the full timeline shows WITHOUT presenting the estimate as fact.
  valueHistory: Array<{ date: string; value: number; approx: boolean }>; // 'YYYY-MM-DD', aggregate (all portfolios), asc
  ...
  // has no connected portfolios (or none have snapshots yet). retrofit-81: carries `approx` too.
  connectedValueHistory: Array<{ date: string; value: number; approx: boolean }>; // 'YYYY-MM-DD', connected-only, asc

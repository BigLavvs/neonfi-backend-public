...
export { AuthError } from './auth.shared.private.js';
...
export async function register(body: RegisterBody): Promise<{ user: UserDTO }> {
  ...
  void (async () => {
    try {
      ...
      // Log the URL so dev can test without waiting for email delivery. Gated on the explicit
      // DEBUG_AUTH_URLS opt-in (NOT NODE_ENV — audit SEC #13): the URL carries a single-use
      // verification token and would let anyone with log access verify any user's email (A11).
      if (config.DEBUG_AUTH_URLS) {
...
export async function resendVerification(body: ResendVerificationBody): Promise<void> {
  ...
  void (async () => {
    try {
      ...
      await redis.set(`email_verify:${token}`, String(user.id), 'EX', 86400);
      const verificationUrl = `${config.APP_BASE_URL}/verify-email?token=${token}`;
      // Explicit DEBUG_AUTH_URLS opt-in only — see A11 / audit SEC #13 note in register().
      if (config.DEBUG_AUTH_URLS) {
...
export { listSessions, revokeSessionById, issueWsTicket, type SessionItem } from './auth.sessions.private.js';
